...
const columns = [
    ...
    { field: 'modified', headerName: 'Last Modified', flex: 0.50 },
    {
        ...
        flex: 0.5,
        renderCell: (params) => (
            <Button onClick={() => handleDownload(params.row.name)}>Download</Button>
        ),
    },
    {
        ...
        flex: 0.5,
        renderCell: (params) => (
            <Button onClick={() => handleValidate(params.row.name, params.row.uploadedBy)}>Validate</Button>
        ),
    },
];
...
function ValidatorTable({ data, loading }) {
